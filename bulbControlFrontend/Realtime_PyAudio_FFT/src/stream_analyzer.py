...
class Stream_Analyzer:
    ...
    def __init__(self,
        ...
        # Custom settings:
        # The axis range of the FFT features will adapt dynamically using a window of N seconds
        self.rolling_stats_window_s = 20
        # [0-1] --> gradually rescales all FFT features to have the same mean
        self.equalizer_strength = 0.20
        # Apply a postprocessing smoothing filter over the FFT outputs
        ...
        self.FFT_window_size = round_up_to_even(
        ...
        self.fftx = np.arange(int(self.FFT_window_size/2),
                              dtype=float) * self.rate / self.FFT_window_size
        ...
        if self.smoothing_length_ms > 0:
            ...
            self.feature_buffer = numpy_data_buffer(len(self.smoothing_kernel), len(
                self.fft), dtype=np.float32, data_dimensions=2)

        # This can probably be done more elegantly...
        self.fftx_bin_indices = np.logspace(np.log2(len(self.fftx)), 0, len(
            self.fftx), endpoint=True, base=2, dtype=None) - 1
        self.fftx_bin_indices = np.round(((self.fftx_bin_indices - np.max(
            self.fftx_bin_indices))*-1) / (len(self.fftx) / self.n_frequency_bins), 0).astype(int)
        self.fftx_bin_indices = np.minimum(np.arange(len(
            self.fftx_bin_indices)), self.fftx_bin_indices - np.min(self.fftx_bin_indices))
        ...
        for bin_index in range(self.n_frequency_bins):
            bin_frequency_indices = np.where(
                self.fftx_bin_indices == bin_index)
        ...
        self.fft_fps = 30
        # Plot log(FFT features) instead of FFT features --> usually pretty bad
        ...
        self.power_normalization_coefficients = np.logspace(np.log2(1), np.log2(
            np.log2(self.rate/2)), len(self.fftx), endpoint=True, base=2, dtype=None)
        self.rolling_stats_window_n = self.rolling_stats_window_s * \
            self.fft_fps  # Assumes ~30 FFT features per second
    ...
    def update_features(self, n_bins=3):
        ...
        self.fft = getFFT(latest_data_window, self.rate,
                          self.FFT_window_size, log_scale=self.log_features)
    ...
    def get_audio_features(self):
        ...
        if self.stream_reader.new_data:  # Check if the stream_reader has new audio data we need to process
            ...
            if self.verbose:
                ...
                avg_data_capture_delay = 1000. * \
                    np.mean(np.array(self.stream_reader.data_capture_delays))
                data_fps = self.stream_reader.num_data_captures / \
                    (time.time() - self.stream_reader.stream_start_time)
                print("Avg fft  delay: %.2fms  -- avg data delay: %.2fms Num data captures: %d (%.2ffps)-- num fft computations: %d (%.2ffps)" %
                      (avg_fft_delay, avg_data_capture_delay, self.stream_reader.num_data_captures, data_fps, self.num_ffts, self.fft_fps), end='\r')
