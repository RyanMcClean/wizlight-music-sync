...
class PlaywrightTests(StaticLiveServerTestCase):
    ...
    def test_load_index(self):
        ...
        try:
            ...
            expect(chromiumPage.locator("#main-title")).to_be_visible(); LOGGER.debug("Main title is visible")
            expect(chromiumPage.locator("#find-bulbs-button")).to_be_visible(); LOGGER.debug("Find bulbs button is visible")
            expect(chromiumPage.locator("#home-link")).to_be_visible(); LOGGER.debug("Home link is visible")
            expect(chromiumPage.locator("#faq-link")).to_be_visible(); LOGGER.debug("FAQ link is visible")
            expect(chromiumPage.locator("#about-link")).to_be_visible(); LOGGER.debug("About link is visible")
    ...
    def test_from_index_to_discover(self):
        ...
        try:
            ...
            self.assertIn("discover", url); LOGGER.debug("URL contains discover")
            self.assertEqual(chromiumPage.title(), "Bulb Bop"); LOGGER.debug("Page title is Bulb Bop")
            expect(chromiumPage.locator("#main-title")).to_be_visible(); LOGGER.debug("Main title is visible")
            expect(chromiumPage.locator("#find-bulbs-button")).to_be_visible(); LOGGER.debug("Find bulbs button is visible")
            expect(chromiumPage.locator("#home-link")).to_be_visible(); LOGGER.debug("Home link is visible")
    ...
    def test_from_discover_to_index(self):
        ...
        try:
            ...
            chromiumPage.locator("#find-bulbs-button").wait_for(timeout=0, state="attached") 
            url = chromiumPage.url 
            self.assertNotIn("discover", url); LOGGER.debug("URL does not contain discover")
            self.assertEqual(chromiumPage.title(), "Bulb Bop"); LOGGER.debug("Page title is Bulb Bop")
            if chromiumPage.locator(".bulb-icon"):
                expect(chromiumPage.locator("#error-toast")).to_have_count(0); LOGGER.debug("No error toast visible")
            else:
                expect(chromiumPage.locator("#error-toast")).to_be_visible(); LOGGER.debug("Error toast is visible")
            expect(chromiumPage.locator("#main-title")).to_be_visible(); LOGGER.debug("Main title is visible")
            expect(chromiumPage.locator("#find-bulbs-button")).to_be_visible(); LOGGER.debug("Find bulbs button is visible")
            expect(chromiumPage.locator("#home-link")).to_be_visible(); LOGGER.debug("Home link is visible")
            expect(chromiumPage.locator("#faq-link")).to_be_visible(); LOGGER.debug("FAQ link is visible")
